...
interface ZapContentProps {
  ...
  setSenderLnAddress: (value: string) => void;
  inputRef: React.RefObject<HTMLInputElement | null>;
  zap: (amount: number, comment: string, senderLnAddress?: string) => void;
    ...
    {invoice ? (
      ...
      <>
        <div className="grid gap-3 px-4 py-4 w-full overflow-hidden">
          ...
          {!isLoggedIn && (
            <Input
              id="sender-ln-address"
              type="text"
              placeholder="Your Lightning address (optional)"
              value={senderLnAddress}
              onChange={(e) => setSenderLnAddress(e.target.value)}
              className="w-full"
            />
          )}
          <Textarea
...
export function ZapDialog({ target, children, className }: ZapDialogProps) {
  ...
  const [senderLnAddress, setSenderLnAddress] = useState<string>('');
  ...
  useEffect(() => {
    if (target) {
      ...
      setComment(isLoggedIn ? 'Zapped via Inkwell!' : '');
    }
  }, [target, isLoggedIn]);
  ...
  const handleZap = () => {
    const finalAmount = typeof amount === 'string' ? parseInt(amount, 10) : amount;
    zap(finalAmount, comment, isLoggedIn ? undefined : senderLnAddress.trim() || undefined);
  };
  ...
  // Show for logged-out users too (guest zapping), but hide if no target,
  // self-authored, or author has no Lightning address
  if (!target || user?.pubkey === target.pubkey || (!author?.metadata?.lud06 && !author?.metadata?.lud16)) {
    return null;
  }
            ...
            <DrawerDescription className="text-sm wrap-break-word text-center">
              {invoice ? (
                ...
                <>
                  ...
                  {!isLoggedIn && ' No Nostr account needed — just enter your Lightning address (optional) and pay with any wallet.'}
                </>
              )}
            </DrawerDescription>
          </DrawerHeader>

...
export function ZapButton({
  ...
  // Don't show zap button if no target, is the author, or author has no lightning address.
  // Logged-out users CAN zap (guest mode), so we don't hide for missing user.
  if (!target || user?.pubkey === target.pubkey || (!author?.metadata?.lud16 && !author?.metadata?.lud06)) {
    return null;
  }

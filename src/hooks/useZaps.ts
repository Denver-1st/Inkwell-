...
export function useZaps(
  ...
  const { data: zapEvents, ...query } = useQuery<NostrEvent[], Error>({
    ...
    queryFn: async () => {
      ...
      if (actualTarget.kind >= 30000 && actualTarget.kind < 40000) {
        ...
        // Profile zap — query by pubkey (p tag)
        const events = await nostr.query([{
          kinds: [9735],
  ...
  const zap = async (amount: number, comment: string, senderLnAddress?: string) => {
    if (amount <= 0) {
      return;
    }
    ...
    try {
      ...
      // Build the LNURL query string.
      // For logged-in users: include a signed NIP-57 zap request (nostr param) so a
      // kind 9735 zap receipt is published by the LNURL service.
      // For guest (logged-out) users: do a plain LNURL-pay with no nostr param.
      // The sender's Lightning address (if provided) is prepended to the comment so
      // the recipient knows who sent the tip.
      let queryString = `amount=${zapAmount}`;

      const guestComment = senderLnAddress
        ? `[${senderLnAddress}] ${comment}`.trim()
        : comment;

      if (user?.signer) {
        // Logged-in: create and sign a NIP-57 zap request
        // For addressable events (30000-39999), pass the object to get 'a' tag.
        // For profile zaps (kind 0), pass undefined — no 'e' tag, just a 'p' tag.
        ...
        queryString += `&nostr=${encodeURI(JSON.stringify(signedZapRequest))}`;
      }

      if (guestComment) {
        queryString += `&comment=${encodeURIComponent(guestComment)}`;
      }

      try {
        const res = await fetch(`${zapEndpoint}?${queryString}`);
